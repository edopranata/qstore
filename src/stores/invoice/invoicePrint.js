...
export const useInvoicePrintStore = defineStore('invoicePrint', {
  ...
  getters: {
    ...
    getSumDOTransaction(state){
      ...
      data.total = state.invoice.data ? state.invoice.data.detail_do ? state.invoice.data.detail_do.reduce((total, next) => parseFloat(total) + parseFloat(next.customer_total), 0) : 0 : 0
      data.count = (14 - data.needed_length) >= 0 ? (14 - data.needed_length) : 1
      data.total_trading = state.invoice.data ? state.invoice.data.detail_trades ? state.invoice.data.detail_trades.reduce((total, next) => parseFloat(total) + parseFloat(next.total), 0) : 0 : 0

      return data
    },
    getSummaries(state){
      const data = {}
      data.total_do = state.invoice.data ? state.invoice.data.detail_do ? state.invoice.data.detail_do.reduce((total, next) => parseFloat(total) + parseFloat(next.customer_total), 0) : 0 : 0
      ...
      const sumaries = []
      sumaries.push(state.invoice.data ? state.invoice.data.detail_do ? state.invoice.data.detail_do.reduce((total, next) => parseFloat(total) + parseFloat(next.customer_total), 0) : 0 : 0)
      sumaries.push(state.invoice.data ? state.invoice.data.detail_trades ? state.invoice.data.detail_trades.reduce((total, next) => parseFloat(total) + parseFloat(next.total), 0) : 0 : 0)
