...
export const useLandCostStore = defineStore('landCost', {
  state: () => ({
    form: {
      edit: false,
      id: null,
  ...
  actions: {
    ...
    async getLandCostDataFromApi(path, startRow, count, filter, sortBy, descending) {
      const data = {
        page: startRow,
        limit: count,
      }
    ...
    async getLandCostData(path, props) {
      ...
      const returnedData = await this.getLandCostDataFromApi(path, page, fetchCount, filter, sortBy, descending)
