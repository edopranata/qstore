...
export const usePlantationRecapReportPrintStore = defineStore('PlantationReportPrintStore', {
  state: () => ({
    form: {
      type: 'Period',
      period_start: null,
  ...
  getters: {
    ...
    getSummaries(state) {
      ...
      data.cost_total = parseFloat(state.table.data ? state.table.data.reduce((total, next) => total + parseFloat(next.gross_total), 0) : 0)
      data.gross_total = parseFloat(state.table.data ? state.table.data.reduce((total, next) => total + parseFloat(next.cost_total), 0) : 0)
