...
export const useTradingRecapReportPrintStore = defineStore('TradingRecapReportPrint', {
  state: () => ({
    form: {
      type: 'Period',
      period_start: null,
  ...
  getters: {
    ...
    getSummaries(state) {
      ...
      data.cost_total = parseFloat(state.table.data ? state.table.data.reduce((total, next) => total + parseFloat(next.cost_total), 0) : 0)
      data.customer_average = parseFloat(state.table.data ? state.table.data.reduce((total, next) => total + parseFloat(next.customer_average), 0) / parseFloat(state.table.data.length) : 0)
      data.customer_total = parseFloat(state.table.data ? state.table.data.reduce((total, next) => total + parseFloat(next.customer_total), 0) : 0)
      data.customer_weight = parseFloat(state.table.data ? state.table.data.reduce((total, next) => total + parseFloat(next.customer_weight), 0) : 0)
