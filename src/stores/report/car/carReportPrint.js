...
export const useCarReportPrintStore = defineStore('carReportPrint', {
  state: () => ({
    form: {
      type: 'Period',
      period_start: null,
  ...
  getters: {
    ...
    getSummaries(state) {
      let data = {}
      let plantation = state.table.data ? state.table.data.filter( next => next.type === 'Plantation') : []
      let trading = state.table.data ? state.table.data.filter(next => next.type === 'Trading') : []
      data.arr_trading = trading
      data.arr_plantation = plantation
      data.tonase = state.table.data ? state.table.data.reduce((total, next) => total + next.net_weight, 0) : 0
      data.plantation = plantation ? plantation.reduce((total, next) => total + next.car_price, 0) : 0
      data.trading = trading ? trading.reduce((total, next) => total + next.car_price, 0) : 0
