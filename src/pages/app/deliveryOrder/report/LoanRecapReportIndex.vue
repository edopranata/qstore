...
const {path} = useRoute()
const report = useDriverLoanRecapReportStore()
const {table, form} = useDriverLoanRecapReportStore()
const {getReportOptions: options, getTypeChange, errors} = storeToRefs(useDriverLoanRecapReportStore())
        ...
        <q-card-section class="tw-space-y-4">
          ...
          <q-tab-panels v-model="form.type" animated class="shadow-2 rounded-borders">
            <q-tab-panel name="Monthly">
              <q-card flat>
                <q-card-section class="no-padding tw-mb-4">
                  <div class="text-h6">Laporan Bulanan Rekapitulasi Angsuran dan Pinjaman Pengepul</div>
                </q-card-section>
                ...
                <q-card-actions class="no-padding tw-mt-4">
                  <div class="md:tw-col-span-4 tw-col-span-2 tw-space-x-2">
                    <q-btn v-if="can('app.deliveryOrder.laporan.rekapPinjamanPengepul')" :disable="table.loading"
                           :loading="table.loading" color="primary"
                           glossy
                           label="Lihat laporan"
                           type="submit"/>
                    <q-btn v-if="can('app.deliveryOrder.laporan.printRekapPinjamanPengepul')"
                           :disable="table.loading || !form.type || String(form.monthly).length !== 7"
                           :loading="table.loading"
                           :to="{name: 'app.deliveryOrder.laporan.printRekapPinjamanPengepul', query: {type: form.type, monthly: form.monthly}}"
                           color="deep-orange"
            ...
            <q-tab-panel name="Annual">
              <q-card flat>
                <q-card-section class="no-padding tw-mb-4">
                  <div class="text-h6">Laporan Tahunan Rekapitulasi Angsuran dan Pinjaman Pengepul</div>
                </q-card-section>
                <q-card-section class="no-padding">
                  <div class="tw-flex tw-flex-col md:tw-grid md:tw-grid-cols-2">
                    <div class="tw-grid tw-grid-cols-2 tw-gap-4">
                ...
                <q-card-actions class="no-padding tw-mt-4">
                  <div class="md:tw-col-span-4 tw-col-span-2 tw-space-x-2">
                    ...
                    <q-btn v-if="can('app.deliveryOrder.laporan.printRekapPinjamanPengepul')"
                           ...
                           :loading="table.loading"
                           :to="{name: 'app.deliveryOrder.laporan.printRekapPinjamanPengepul', query: {type: form.type, annual: form.annual}}"
                           color="deep-orange"
                           glossy icon="print" label="Print laporan"/>
                  </div>
                </q-card-actions>
        ...
        <q-card-section>
          <q-markup-table>
            ...
            <tr>
              ...
              <th class="text-left">Periode / Tanggal</th>
              <th class="text-left">Jumlah Trx</th>
              <th class="text-right">Pinjam (Rp)</th>
              <th class="text-right">Angsuran (Rp)</th>
            </tr>
            </thead>
