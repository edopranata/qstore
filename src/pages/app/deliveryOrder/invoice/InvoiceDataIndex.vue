<script setup>
import {useInvoiceCollectorDataStore} from "stores/invoice/invoiceCollectorData";
...
const invoice = useInvoiceCollectorDataStore()
const {table} = useInvoiceCollectorDataStore()
const {path} = useRoute()
...
const toPrint = async (invoice) => {
  if (invoice)
    $q.dialog({
      ...
      await router.replace({name: 'app.deliveryOrder.dataInvoicePengepul.printInvoice', params: {invoice_number: invoice}})
    })
}
