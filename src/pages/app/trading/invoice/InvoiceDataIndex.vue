<script setup>
import {useInvoiceFarmerDataStore} from "stores/invoice/invoiceFarmerData";
...
const invoice = useInvoiceFarmerDataStore()
const {table} = useInvoiceFarmerDataStore()
const {path} = useRoute()
...
const toPrint = async (invoice) => {
  if (invoice)
    $q.dialog({
      ...
      await router.replace({name: 'app.jualBeliSawit.dataInvoicePetani.printInvoice', params: {invoice_number: invoice}})
    })
}
