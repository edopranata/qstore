<script setup>
import {useCarCostStore} from "stores/cost/carCost";
...
const {path} = useRoute()
const cost = useCarCostStore()
const {table, form, select} = useCarCostStore()
const {errors, getSelectedCar, getSelectedCostType, getSelected: selected, form: formField} = storeToRefs(useCarCostStore())
const $q = useQuasar()
const tableRef = ref()

onMounted(async () => {
  await cost.loadAllCars(path)
  tableRef.value.requestServerInteraction()
})

watch([getSelectedCar, getSelectedCostType], ([selectedCar, selectedCostType]) => {
  form.car_id = selectedCar ? selectedCar.id : selectedCar
...
const onRequest = async (props) => {
  await cost.getCarCostData(path, props)
}

const searchCar = (val, update) => {
  update(() => {
    const cars = select.cars
    if (val !== '') {
      const needle = val.toLowerCase()
      select.cars_option = cars.filter(({no_pol}) => no_pol.toLowerCase().indexOf(needle) > -1)
    } else {
      select.cars_option = cars.slice(0, 10)
    }
  })
}
...
const onUpdate = () => {
  $q.dialog({
    ...
    for (let property in form) {
      ...
      if (property === 'car_id') {
        form.car_id = sel.subject_id
        let c = select.cars.filter(car => car.id === form.car_id)
        select.selected_car = c[0]
      }
        ...
        >
          <template v-slot:top-row>
            <q-tr>
              ...
              <q-th></q-th>
              <q-th>
                <q-input v-model="table.search.no_pol" debounce="300" dense label="Plat no"/>
              </q-th>
              <q-th>
                <q-select
          ...
          <template v-slot:body-cell-subject="props">
            <q-td>
              <div class="tw-font-bold">{{ props.row?.subject.name }}</div>
              <div class="tw-text-xs">{{ props.row?.subject.no_pol }}</div>
            </q-td>
          </template>
