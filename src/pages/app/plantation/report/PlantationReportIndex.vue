...
import {usePlantationReportStore} from "stores/report/plantation/plantationReportStore";
...
<template>
  <q-page class="tw-space-y-4" padding>
    ...
    >
      <q-card bordered>
        <q-card-section class="tw-space-y-4 print-hide">
          <q-tabs
            v-model="form.type"
          ...
          <q-tab-panels v-model="form.type" animated class="shadow-2 rounded-borders">
            <q-tab-panel name="Period">
              <q-card flat>
                <q-card-section class="no-padding tw-mb-4">
                  <div class="text-h6">Periode Laporan Pendapatan Jual Beli Sawit</div>
                </q-card-section>
                ...
                <q-card-actions class="no-padding tw-mt-4">
                  <div class="md:tw-col-span-4 tw-col-span-2 tw-space-x-2">
                    <q-btn v-if="can('app.perkebunan.laporan.perkebunan')" :disable="table.loading"
                           ...
                           type="submit"/>
                    <q-btn
                      v-if="can('app.perkebunan.laporan.printPerkebunan')"
                      :disable="table.loading || !form.type || !form.period_start || !form.period_end"
                      :loading="table.loading"
                      :to="{name: 'app.perkebunan.laporan.printPerkebunan', query: {type: form.type, period_end: form.period_end, period_start: form.period_start }}"
                      color="deep-orange"
            ...
            <q-tab-panel name="Monthly">
              <q-card flat>
                <q-card-section class="no-padding tw-mb-4">
                  <div class="text-h6">Laporan Bulanan Pendapatan Jual Beli Sawit</div>
                </q-card-section>
                <q-card-section class="no-padding">
                  <div class="tw-flex tw-flex-col md:tw-grid md:tw-grid-cols-2">
                    <div class="tw-grid tw-grid-cols-2 tw-gap-4">
                ...
                <q-card-actions class="no-padding tw-mt-4">
                  <div class="md:tw-col-span-4 tw-col-span-2 tw-space-x-2">
                    <q-btn v-if="can('app.perkebunan.laporan.perkebunan')" :disable="table.loading"
                           ...
                           label="Lihat laporan"
                           type="submit"/>
                    <q-btn v-if="can('app.perkebunan.laporan.printPerkebunan')"
                           :disable="table.loading || !form.type || String(form.monthly).length !== 7"
                           :loading="table.loading"
                           :to="{name: 'app.perkebunan.laporan.printPerkebunan', query: {type: form.type, monthly: form.monthly}}"
                           color="deep-orange"
                           glossy icon="print" label="Print laporan"/>
                  </div>
                </q-card-actions>
            ...
            <q-tab-panel name="Annual">
              <q-card flat>
                <q-card-section class="no-padding tw-mb-4">
                  <div class="text-h6">Laporan Tahunan Pendapatan Jual Beli Sawit</div>
                </q-card-section>
                <q-card-section class="no-padding">
                  <div class="tw-flex tw-flex-col md:tw-grid md:tw-grid-cols-2">
                    <div class="tw-grid tw-grid-cols-2 tw-gap-4">
                ...
                <q-card-actions class="no-padding tw-mt-4">
                  <div class="md:tw-col-span-4 tw-col-span-2 tw-space-x-2">
                    ...
                    <q-btn v-if="can('app.perkebunan.laporan.printPerkebunan')"
                           ...
                           :loading="table.loading"
                           :to="{name: 'app.perkebunan.laporan.printPerkebunan', query: {type: form.type, annual: form.annual}}"
                           color="deep-orange"
                           glossy icon="print" label="Print laporan"/>
                  </div>
                </q-card-actions>
          ...
          <q-markup-table>
            ...
            <template v-for="( detail, i) in table.data" :key="i">
              <tr>
                ...
                <tr v-for="(land, l) in detail.details" :key="l">
                  <td colspan="3" class="text-right">{{ `${land?.land?.name} (Luas : ${land.wide} Ha)` }} </td>
                  <td class="text-right">
                    {{
                      new Intl.NumberFormat('id-ID', {
                        style: 'unit',
                        unit: 'kilogram'
                      }).format(parseFloat(detail.wide_weight * land.wide))
                    }}
                  </td>
                  <td class="text-right">
                    {{
                  ...
                  <td class="text-right">
                    {{
                      ...
                      currency: 'IDR',
                      maximumFractionDigits: 2
                      }).format(parseFloat(detail.wide_cost * land.wide))
                    }}
                  </td>
                  <td class="text-right">
                    {{
                      ...
                      currency: 'IDR',
                      maximumFractionDigits: 2
                      }).format(parseFloat(detail.wide_weight * land.wide) * parseFloat(detail.net_price))
                    }}
                  </td>
                  <td class="text-right">
                    {{
                      ...
                      currency: 'IDR',
                      maximumFractionDigits: 2
                      }).format(parseFloat(parseFloat(detail.wide_weight * land.wide) * parseFloat(detail.net_price)) - parseFloat(detail.wide_cost * land.wide))
                    }}
                  </td>
                </tr>
            </template>
