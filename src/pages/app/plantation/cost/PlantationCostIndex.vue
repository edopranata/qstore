<script setup>
import {useLandCostStore} from "stores/cost/landCost";
...
const {path} = useRoute()
const cost = useLandCostStore()
const {table, form, select} = useLandCostStore()
const {errors, getSelectedLand, getSelectedCostType, getSelected: selected, form: formField,} = storeToRefs(useLandCostStore())
const $q = useQuasar()
const tableRef = ref()

onMounted(async () => {
  await cost.loadAllLands(path)
...
const onRequest = async (props) => {
  await cost.getLandCostData(path, props)
}

const searchLand = (val, update) => {
  update(() => {
    const lands = select.lands
    if (val !== '') {
      const needle = val.toLowerCase()
      select.lands_option = lands.filter(({name}) => name.toLowerCase().indexOf(needle) > -1)
    } else {
      select.lands_option = lands.slice(0, 10)
    }
  })
}
    ...
    <q-card bordered>
      ...
      >
        ...
        <q-card-section class="tw-flex justify-between tw-space-x-2">
          <q-btn
            v-if="can('app.perkebunan.biayaKebun.simpanBiayaKebun')"
          ...
          <q-space></q-space>
          <q-btn
            v-if="can('app.perkebunan.biayaKebun.hapusBiayaKebun')"
            :dense="$q.screen.lt.lg"
          ...
          <q-btn
            v-if="can('app.perkebunan.biayaKebun.ubahBiayaKebun')"
            :dense="$q.screen.lt.lg"
            :disable="selected.length !== 1"
        ...
        >
          <template v-slot:top-row>
            <q-tr>
              ...
              <q-th></q-th>
              <q-th>
                <q-input v-model="table.search.name" debounce="300" dense label="Nama kebun"/>
              </q-th>
              <q-th>
                <q-select
          ...
          <template v-slot:body-cell-subject="props">
            <q-td>
              ...
              <div class="tw-text-xs">{{ props.row?.subject.area }}</div>
            </q-td>
          </template>
