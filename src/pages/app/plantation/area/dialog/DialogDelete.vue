...
const {path} = useRoute()
const {dialog, form, submitDelete, table, deleted} = useAreasStore()
...
<template>
  <q-dialog v-model="dialog.delete" persistent transition-hide="scale" transition-show="scale">
    <q-card style="width: 600px;">
      <q-card-section>
        <div class="text-h6">Yakin akan menghapus {{ deleted.data.length }} data area
          ini?
        </div>
      </q-card-section>
      ...
      <q-card-actions align="right">
        ...
        <q-space/>
        <q-btn :disable="table.loading" color="primary" flat label="Delete Data" type="button"
               @click.prevent="submitDelete(path)"/>
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
